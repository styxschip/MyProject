...
{
    TRACE("ssc CThreadPool::WorkThread() begin!!");
            ...
            case 1: //该干活了
                {
                    ...
                    TRACE("ssc pThis->m_TaskLst.GetCommand(0x%08x)", pCmd);
                    if (bRet == FALSE)
                    {
                        TRACE("ssc pThis->m_TaskLst.GetCommand(0x%08x) failed!", pCmd);
                        continue;
                    }
                    pCmd->Exec();
                    TRACE("ssc pCmd->Exec()");
...
BOOL CThreadPool::Handle(ICommand *pCmd)
{
    ...
    TRACE("ssc m_TaskLst.AddTail(%p)", pCmd);
...
{
    TRACE("ssc CThreadPool::Create() begin!!");
