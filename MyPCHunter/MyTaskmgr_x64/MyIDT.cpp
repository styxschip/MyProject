...
DWORD CMyIDT::GetIDTFromSys()
{
    ...
    DWORD dwBytes = 0;
    //从驱动获取IDT长度 理论上是0xfff但是保险起见还是获取读一下好
    ...
    //dwMap = dwIDTL / 8; //不出意外应该是0xfff / 8 = 512 / 2 核心 = 256
    dwMap = 256;
...
//         if(g_szIDTFuncName[i] == NULL)
//         {
//             char sz[MAXBYTE] = {0};
//             wsprintf(sz, "%d", i);
// AfxMessageBox(sz);
//         }   
        wsprintf(szTmpBuf, "%s", g_szIDTFuncName[i%256]);
        m_LstIDT.SetItemText(i, 2, szTmpBuf); //函数名称
        
        CFunction::GetSelectorFormIDT((PIDT)(pParams + j * 16), &dwSelector);
        wsprintf(szTmpBuf, "0x%02X", dwSelector);
        m_LstIDT.SetItemText(i, 3, szTmpBuf); //段选择子
        
        
        CFunction::GetOffsetFormIDT((PIDT)(pParams + j * 16), &qwOffset);
        wsprintf(szTmpBuf, "0x%016X", qwOffset);
...
static char *g_szIDTFuncName[0x100] = {
        "Divide error",
        "Debug",
        "Not used",
        "Breakpoint",
