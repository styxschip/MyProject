...
BOOL CMyGDT::OnInitDialog() 
{
    ...
    OutputDebugString("show GDT over\r\n");
...
DWORD CMyGDT::GetGDTFromSys()
{
    ...
    {
        //判断P位是否有效 有效才显示
        CFunction::GetAttribFormGDT((PGDT)(pParams + j * 16), &wAttrib);
        ...
        {
            CFunction::GetBaseFormGDT((PGDT)(pParams + j * 16), &qwBase);
            ...
            wsprintf(szTmpBuf, "0x%016X", qwBase);
            m_LstGDT.SetItemText(k, 2, szTmpBuf); //基址
            
            CFunction::GetLimiteFormGDT((PGDT)(pParams + j * 16), &dwLimite);
            wsprintf(szTmpBuf, "0x%016X", qwBase + dwLimite);
